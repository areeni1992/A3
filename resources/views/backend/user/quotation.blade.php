...
@section('content')
    ...
    <form  action="{{ route('insertQuotDate', $quotData->id ?? '') }}" method="POST" enctype="multipart/form-data">
        ...
        <div class='row d-flex'>
            @foreach( config('app.languages') as $key => $lang)
                <div class="col-md-6 @if($key == 'ar') {{ 'text-end' }} @endif">
                    ...
                    <div class="form-outline mb-4">
                        ...
                        <textarea class="form-control" id="short_desc" name="{{$key}}[short_desc]">{{ isset($quotData) ? $quotData->translate($key)->short_desc : '' }}</textarea>
                    </div>
                    <!-- body -->
                    <div class="form-outline mb-4">
                        <label class="form-label" for="description">{{ __('words.content') }}</label>
                        <textarea class="form-control" id="myeditorinstance" rows="4" name="{{$key}}[desc]">{{ isset($quotData) ? $quotData->translate($key)->desc : '' }}</textarea>
                    </div>
                </div>
            @endforeach
            ...
            <!-- image -->
            <div class="w-40 h-25">
                <img class="w-100" src="{{ isset($quotData) ? config('app.url').'/storage/images/'.$quotData->background : '' }}" alt="">
            </div>
            <div class="form-outline mb-4">
