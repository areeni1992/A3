...
@section('content')
    ...
    <form action="{{ route('insertQuestin') }}" method="POST" enctype="multipart/form-data">
        @csrf
        <div class='row d-flex'>
            <div class="input-group mb-3">
                ...
                <select name="parent_id" class="custom-select" id="inputGroupSelect01">
                    ...
                    @foreach($faqs as $faq)
                    <option value="{{ $faq->id }}">{{ $faq->translate(app()->getLocale())->faq_name }}</option>
                    @endforeach
                </select>
            </div>
        @foreach( config('app.languages') as $key => $lang)
                <div class="col-md-6 @if($key == 'ar') {{ 'text-end' }} @endif">
                    ...
                    <div class="form-outline mb-4">
                        ...
                        <input type="text" id="{{$key}}[question]" class="form-control" name="{{$key}}[question]"/>
                    </div>
                    <!-- answer  -->
                    <div class="form-outline mb-4">
                        <label class="form-label" for="{{$key}}[answer]"> Answer </label>
                        <textarea id="{{$key}}[answer]" class="form-control" name="{{$key}}[answer]" cols="30" rows="10"></textarea>
                    </div>
                </div>
            @endforeach
        </div>
